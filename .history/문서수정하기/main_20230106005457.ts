...
function secondStepMoveCloneNode(cloneNode: HTMLElement, position: string) {
  ...
  //   position === 'afterbegin'
  // ) {
  //   if (cTop - top > 0) 이동시킬Y = -Math.abs(cTop - top);
  //   else 이동시킬Y = -Math.abs(cTop - top);
  // } else {
  //   이동시킬Y = -Math.abs(cTop - bottom);
  // }
  // console.log(이동시킬Y, 'yy');
  // console.log(cloneNode.getBoundingClientRect());
  // console.log('55라인');
  console.log(cTop, top);
  console.log(이동시킬Y);
  cloneNode.style.transform = `translate(-${이동시킬X}px, ${255}px)`;
  // console.log(cloneNode.getBoundingClientRect());
  // setTimeout(() => {
...
function fisrstStepMoveCloneNode(cloneNode: HTMLElement, position: string) {
  ...
  if (
    ...
    cloneNode.style.transform = 'translateY(-300px)';
  } else {
    cloneNode.style.transform = 'translateY(300px)';
  }
}
